...
class MetaNet(torch.nn.Module):
    def __init__(self, param_dict):
        ...
        self.hidden = torch.nn.Linear(1920, 128*self.param_num)
        self.fc_dict = {}
        for i, (name, params) in enumerate(param_dict.items()):
            self.fc_dict[name] = i
            setattr(self, 'fc{}'.format(i+1), torch.nn.Linear(128, params))
    
    def forward(self, mean_std_features):
        hidden = torch.nn.functional.relu(self.hidden(mean_std_features))
        filters = {}
